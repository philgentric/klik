...
import klikr.browser_core.items.Item;
...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        if (source == null)
        {
            logger.log((Logger.warning+" WARNING: accept_drag_dropped_as_a_move_in, cannot check for stupid move because the event's source is null: " + drag_event.getSource()));
        }
        else
        {
            if (source == excluded) {
                logger.log(Logger.warning+" source is excluded: cannot drop onto itself");
                drag_event.consume();
                return 0;
            }
