...
public class PCA extends FeatureExtraction {
	...
	public static FeatureExtraction get_PCA(
			ArrayList<Matrix> trainingSet, ArrayList<String> labels,
			   int numOfComponents) throws Exception {

		System.out.println("numOfComponents=" + numOfComponents);
		System.out.println("trainingSet.size()=" + trainingSet.size());

		if (numOfComponents >= trainingSet.size()) {
			System.out.println("trainingSet.size() is TOO SMALL");
			return null; //throw new Exception("the expected dimensions could not be achieved!");
		}
