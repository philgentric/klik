...
//**********************************************************
{
    ...
    public final Path the_play_list_file_path;
    public final List<String> paths = new ArrayList<>();
    public final Logger logger;

    //**********************************************************
    public Playlist_path_list_provider(String name, Logger logger)
    //**********************************************************
    {
        this.logger = logger;

        the_play_list_file_path = Path.of(name);
        try {
            List<String> ss = Files.readAllLines(the_play_list_file_path);
            for ( String s : ss)
            {
                paths.add(s);
            }
        ...
        {
            logger.log("No such file: "+the_play_list_file_path);
        }
        catch (IOException e) {
    ...
    //**********************************************************
    {
        return the_play_list_file_path.toAbsolutePath().toString();
    }
    ...
    //**********************************************************
    {
        try {
            ...
            Files.write(the_play_list_file_path,paths,java.nio.charset.StandardCharsets.UTF_8, StandardOpenOption.CREATE);
        }
        catch (IOException e) {
            logger.log(Stack_trace_getter.get_stack_trace(e.toString()));
        }
