...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Pane top_pane;
        if (use_button) {
            top_pane = define_top_bar_using_buttons_deep(height, up_button, trash);
        } else {
            top_pane = define_top_bar_as_mix_of_buttons_and_menus(height, up_button, trash);
        }
    ...
    /*
    variant using a menu bar for the top
    beware there is a lot of copy/pasted code here
    this is due to the fact that ContextMenu and menuBar
    have the same syntax but do not have a meaningfull
    common class in the hierachy
     */
    //**********************************************************
    private MenuBar define_menu_bar(double top_button_height)
    //**********************************************************
    {
        MenuBar menu_bar = new MenuBar();
        Look_and_feel_manager.set_menubar_look(menu_bar);

        menu_bar.setPrefHeight(top_button_height);
        {
            {
                String bandh = I18n.get_I18n_string("Bookmarks", logger);
                bandh += " & " + I18n.get_I18n_string("History", logger);
                Menu bandh_menu = new Menu(bandh);
                bandh_menu.getItems().add(browser_menus.make_bookmarks_menu());
                bandh_menu.getItems().add(browser_menus.make_history_menu());
                menu_bar.getMenus().add(bandh_menu);
            }

            {
                Menu files_menu = define_files_menu_items();
                menu_bar.getMenus().add(files_menu);
            }
            {
                String view = I18n.get_I18n_string("View", logger);
            ...
            {
                Menu pref = define_menu_preferences();
                menu_bar.getMenus().add(pref);
            }
            browser.always_on_front_nodes.add(menu_bar);
        }
        return menu_bar;
    }

    //**********************************************************
    private Pane define_top_bar_as_mix_of_buttons_and_menus(double top_button_height, Button go_up, Button trash)
    //**********************************************************
    {
        Pane top_pane;
        top_pane = new BorderPane();
        MenuBar menu_bar = define_menu_bar(top_button_height);
        ((BorderPane) top_pane).setCenter(menu_bar);

        /*
        if (style_sheet_url_string != null) {
            top_pane.getStylesheets().clear();
            top_pane.getStylesheets().add(style_sheet_url_string);
            top_pane.getStyleClass().add(LOOK_AND_FEEL_GENERAL);
        }*/
        ((BorderPane) top_pane).setLeft(go_up);
        ((BorderPane) top_pane).setRight(trash);
        return top_pane;
    }

    //**********************************************************
    private Menu define_files_menu_items()
    //**********************************************************
    {
        String files = I18n.get_I18n_string("Files", logger);
    ...
    //**********************************************************
    private Menu define_menu_preferences()
    //**********************************************************
    {
        String preferences = I18n.get_I18n_string("Preferences", logger);
