...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        vbox = new VBox();
        {
            ...
            for (int i = 0; i < how_many_rectangles; i++)
            {
                ...
                Rectangle r = new Rectangle(x, Meters_stage.DISPLAY_PIXEL_HEIGHT-y, w, y);
                r.setManaged(false);
                x += w + gap;
                r.setFill(color);
            ...
            create_h_scale(min,max,largest_count);
    ...
    private void create_h_scale(double min, double max, int largest_count)
    {

        the_hbox.getChildren().remove(min_x);
        double y = val_to_pix(largest_count, 0);
        min_x = new Rectangle(x_offset+LEFT+ADJUST, Meters_stage.DISPLAY_PIXEL_HEIGHT-y, 3, 20);
        ...
        min_x_text.setY(Meters_stage.DISPLAY_PIXEL_HEIGHT-10);
        the_hbox.getChildren().add(min_x_text);


        the_hbox.getChildren().remove(max_x);
        max_x = new Rectangle(x_offset+LEFT+ADJUST +how_many_rectangles *(w+gap), Meters_stage.DISPLAY_PIXEL_HEIGHT-y, 3, 20);
        ...
        max_x_text.setY(Meters_stage.DISPLAY_PIXEL_HEIGHT-10);
    ...
    {
        return ((double) val)*Meters_stage.DISPLAY_PIXEL_HEIGHT/ (double)largest_count;
    }
    ...
    //**********************************************************
    private void create_v_scale(int largest_count)
    //**********************************************************
    {
