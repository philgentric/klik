...
import klik.machine_learning.song_similarity.Feature_vector_bitmap;
...
//**********************************************************
public class Feature_vector_cache
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Path dir = get_feature_vector_cache_dir(null, aborter,logger);
        cache_file_path= Path.of(dir.toAbsolutePath().toString(), cache_file_name);
        if ( dbg) logger.log(tag +" cache file ="+cache_file_path);
        feature_vector_creation_actor = new Feature_vector_creation_actor(fvs,aborter);
    ...
    //**********************************************************
    public static Path get_feature_vector_cache_dir(Stage owner, Aborter aborter, Logger logger)
    //**********************************************************
    {
        ...
        try(DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(cache_file_path.toFile()))))
        {
            ...
            for ( int i = 0; i < number_of_vectors; i++)
            {
                ...
                if ( type == feature_vector_is_string)
                {
                    ...
                    fv = new Feature_vector_bitmap(s,logger);
                }
                if ( fv == null)
                {
    ...
    //**********************************************************
    {
        ...
        try(DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(cache_file_path.toFile()))))
        {
            ...
            if ( e0.getValue() instanceof Feature_vector_bitmap)
            {
            ...
            for(Map.Entry<String, Feature_vector> e : path_to_feature_vector_cache.entrySet())
            {
                ...
                {
                    Feature_vector_bitmap fvb = (Feature_vector_bitmap) fv;
    ...
    //**********************************************************
    {
        logger.log("update "+paths+" fv to be rebuild");
