...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Non_booleans.get_main_properties_manager(logger).store_properties();
    ...
    //**********************************************************
    {
        ...
        for(int i = 0 ; i <= 12 ; i++)
        {
            ...
            String s = (String) Non_booleans.get_main_properties_manager(logger).get(key);
            if ( s == null ) break;
            if ( s.equals(source) == true)
            {
                key = LAST_DESTINATION_DIR +i;
                s = (String) Non_booleans.get_main_properties_manager(logger).get(key);
                if ( s == null ) break;
                if ( s.equals(destination) == true)
                {
                    // FOUND
                    key = LAST_STATUS +i;
                    Non_booleans.get_main_properties_manager(logger).raw_put(key,status);
                }
            }
        }
    }
    ...
    //**********************************************************
    {
        ...
        for(int j = 0 ; j < 12; j++)
        {
            ...
            if ( s.equals(absolutePath_source) == true)
            {
                ...
                String s2 = (String) Non_booleans.get_main_properties_manager(logger).get(key);
                if ( s2 == null ) break;
                if ( s2.equals(absolutePath_destination) == true)
                {
                    ...
                    Date d = new Date();
                    s = d.toString();
                    Non_booleans.get_main_properties_manager(logger).raw_put(key,s);
                    return;
                }
            }
        }
        ...
        {
            key = LAST_SOURCE_DIR +j;
            String s = Non_booleans.get_main_properties_manager(logger).get(key);
            if ( s == null ) continue;
            key = LAST_SOURCE_DIR +(j+1);
            Non_booleans.get_main_properties_manager(logger).raw_put(key,s);

            key = LAST_DESTINATION_DIR +j;
            s = Non_booleans.get_main_properties_manager(logger).get(key);
            if ( s == null )
            {
                // this is BAD !break;
        ...
        Non_booleans.get_main_properties_manager(logger).raw_put(LAST_DESTINATION_DIR+"0", absolutePath_destination);
        Non_booleans.get_main_properties_manager(logger).raw_put(LAST_SOURCE_DIR+"0", absolutePath_source);
        Date d = new Date();
        String s = d.toString();
        Non_booleans.get_main_properties_manager(logger).raw_put(LAST_SAVE_DATE+"0", s);
    ...
    //**********************************************************
    {
        ...
        for(int j = i; j <=12 ; j++)
        {
            ...
            if ( s == null )
            {
                ...
                if ( s != null )
                {
                    Non_booleans.get_main_properties_manager(logger).remove(key);
                }
                key = LAST_DESTINATION_DIR +j;
                s = (String) Non_booleans.get_main_properties_manager(logger).get(key);
                if ( s != null )
