...
//**********************************************************
{
    ...
    //**********************************************************
    {
        ...
        Image image = null;
        try(InputStream input_stream = Full_image_from_disk.get_image_InputStream(original_image_file, Feature_cache.get(Feature.Fusk_is_on), report_if_not_found, aborter,logger))
        {
...
/*
            switch ( item_type)
            {
                ...
                default -> {
                    logger.log(Stack_trace_getter.get_stack_trace("Icons_from_disk WARNING: unexpected item_type "+ item_type+ " for "+original_image_file));
                    return Optional.empty();
                }
            }
*/
    ...
    //**********************************************************
    private static Optional<Image> use_GraphicsMagick_for_icon(Path original_image_file, double icon_size, Window owner, Logger logger)
    //**********************************************************
    {
    ...
    /*
    //**********************************************************
    private static Optional<Image> use_fits_NASA(Path original_image_file, double icon_size, Aborter aborter, Window owner, Logger logger)
    //**********************************************************
    {
        logger.log("use_fits_NASA");

        Optional<Image> op = FITS.load_FITS_image(original_image_file, aborter, owner,logger);
...
*/
    ...
    //**********************************************************
    {
        ...
        if ( image.isError())
        {
            ...
            //if ( dbg)
            //logger.log(Stack_trace_getter.get_stack_trace("Icons_from_disk WARNING: an error occurred when reading AND resizing: "+original_image_file.toAbsolutePath()));
            //image = Icons_from_disk.load_native_resolution_image_from_disk(original_image_file, true, null, aborter,logger);
        }
        return Optional.of(image);
